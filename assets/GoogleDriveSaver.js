...
GDS.prototype.load = function(filename) {
	this.debug && console.log('GDS.load', filename);
	var self = this;
	...
	var prm = new Promise(function(resolve, reject) {
		if ( loaded === true && self.authChecked === true && self.fileMetas[filename] ) {
			return self.updateFile(self.fileMetas[filename], data)
				.then(function(data) {
					this.debug && console.log('GDS.save success', filename);
					resolve(data)
				})
				.catch(function(err) {
					this.debug && console.log('GDS.save failed', filename, err);
					reject(err);
				})
				;
		} else {
...
GDS.prototype.deleteFile = function(file_id) {
	this.debug && console.log('GoogleSaver deleteFile', file_id);
	var self = this;
...
GDS.prototype.newSaveFile = function(filename) {
	this.debug && console.log('GDS.newSaveFile', filename);
	var self = this;
